...
with c2:
    ...
    if monthly.empty:
        ...
        fig.update_traces(texttemplate="%{text:.0f} mm", textposition="outside", textfont=dict(size=12, color="#f8fafc"), cliponaxis=False, hovertemplate="Estate: %{fullData.name}<br>Bulan: %{x}<br>Curah hujan: %{y:.0f} mm<extra></extra>")
        st.plotly_chart(chart_layout(fig, 360), use_container_width=True)
                ...
                if estate_pie["Jumlah Hari"].sum() == 0:
                    ...
                    fig = go.Figure(
                        data=[go.Pie(
                            ...
                            hovertemplate="%{label}<br>%{value} hari<br>%{percent}<extra></extra>",
                        )]
                    )
                    fig.update_layout(title={"text": f"<b>{estate}</b>", "x": .5, "xanchor": "center", "font": {"size": 18, "color": "#f8fafc"}})
                    st.plotly_chart(chart_layout(fig, 295, show_legend=False), use_container_width=True)
